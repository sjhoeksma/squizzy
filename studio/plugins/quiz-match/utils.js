...
export const assembleMatchUrl = ({slug}) => {
  console.log(window.location.protocol + '//' + window.location.host + `/match/${slug.current}`)
  if (config.default.localWebHost && window.location.hostname === 'localhost') {
    return `${config.default.localWebHost}/match/${slug.current}`
  }
  return window.location.protocol + '//' + window.location.host + `/match/${slug.current}`
}
